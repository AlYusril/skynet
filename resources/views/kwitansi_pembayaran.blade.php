		...
		<div class="invoice-box">
			<table cellpadding="0" cellspacing="0">
				...
				<tr class="information">
					<td colspan="3">
						<table>
							<tr>
								<td>
									{{ settings()->get('app_name') }}<br />
									{{ settings()->get('app_address') }}<br />
									{{ formatNomorHp(settings()->get('app_phone')) }}<br />
								</td>
				...
				<tr class="total" style="background: #eee">
					...
					<td>{{ formatRupiah($pembayaran->tagihan->total_tagihan) }}</td>
				</tr>

				<tr>
					<td colspan="3"><i>Terbilang : {{ ucwords(terbilang($pembayaran->tagihan->total_tagihan)) }} Rupiah</i></td>
				</tr>
			</table>
			<small class="text-muted"><i>Dicetak pada tanggal {{ now()->format('d F Y H:i') }}</i></small>
		</div>
	</body>
</html>