...
<div class="row justify-content-center">
    <div class="col-md-12 grid-margin stretch-card">
        <div class="card">
            <div class="card-body">
                {!! Form::open(['route' => 'laporkerusakan.store', 'method' => 'POST']) !!}
                    ...
                    <tbody>
                        @forelse ($laporan as $item)
                            <tr>
                                ...
                                <td class="text-center">
                                    {!! Form::open([
                                        'route' => ['laporkerusakan.destroy', $item->id],
                                        'method' => 'DELETE',
                                    ...
                                    @if ($item->status === 'Belum')
                                        {!! Form::open([
                                            'route' => ['laporkerusakan.update', $item->id],
                                            'method' => 'PATCH', // Use the HTTP PATCH method
                                            'onsubmit' => 'return confirm("Change status to On-Process?")',
                                        ]) !!}
                                        <button type="submit" name="status" value="On-Process" class="btn btn-primary btn-sm">
                                            On-Process
                                        </button>
                                        {!! Form::close() !!}
                                    @elseif ($item->status === 'On-Process')
                                        {!! Form::open([
                                            'route' => ['laporkerusakan.update', $item->id],
                                            'method' => 'PATCH', // Use the HTTP PATCH method
                                            'onsubmit' => 'return confirm("Change status to Selesai?")',
                                        ]) !!}
